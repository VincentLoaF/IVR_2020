...
class image_converter:
    ...
    def __init__(self):
        ...
        self.robot_blue_pos_est_pub1 = rospy.Publisher("/robot/blue_position_estimation/cam1", Float64MultiArray,
                                                       queue_size=10)
        self.robot_green_pos_est_pub1 = rospy.Publisher("/robot/green_position_estimation/cam1", Float64MultiArray,
                                                        queue_size=10)
        self.robot_red_pos_est_pub1 = rospy.Publisher("/robot/red_position_estimation/cam1", Float64MultiArray,
                                                      queue_size=10)
    ...
    def move_joints(self):
        ...
        self.joint2 = Float64()
        self.joint2.data = float(np.pi / 2 * np.sin(cur_time * np.pi / 15))
        self.joint3 = Float64()
        self.joint3.data = float(np.pi / 2 * np.sin(cur_time * np.pi / 18))
        self.joint4 = Float64()
        self.joint4.data = float(np.pi / 2 * np.sin(cur_time * np.pi / 20))
        try:
    ...
    def callback1(self, data):
        ...
        blue_pos.data = self.pixel2meter(self.cv_image1, self.detect_blue(self.cv_image1))
        green_pos = Float64MultiArray()
        green_pos.data = self.pixel2meter(self.cv_image1, self.detect_green(self.cv_image1))
        ...
        try:
            ...
            self.robot_blue_pos_est_pub1.publish(blue_pos)
